...
namespace detail {
    ...
    // Helper to check if a column is insertable (doesn't have must_not_insert trait)
    template<typename Column>
    constexpr bool is_insertable_column() {
        using traits = typename Column::_traits;
        using tags = typename traits::_tags;
        return !sqlpp::detail::is_element_of<sqlpp::tag::must_not_insert, tags>::value;
    }
    
    // Helper to check if a column spec is insertable (used for compile-time checks)
    template<typename ColumnSpec>
    constexpr bool is_insertable_column_spec() {
    ...
    public:
        ...
        column_proxy& operator=(T&& val) {
            static_assert(is_insertable_column_spec<ColumnSpec>(), 
...
public:
    ...
    void set(T&& value) {
        static_assert(detail::is_insertable_column_spec<ColumnSpec>(), 
