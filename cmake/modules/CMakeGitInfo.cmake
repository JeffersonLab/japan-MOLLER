# Source: https://github.com/andrew-hardin/cmake-git-version-tracking Original
# license: MIT License Current license: MIT License
#
# This file defines the functions and targets needed to monitor the state of a
# git repo. If the state changes (e.g. a commit is made), then a file gets
# reconfigured.
#
# The behavior of this script can be modified by defining any of these
# variables:
#
...
#
# Script design: - This script was designed similar to a Python application with
# a Main() function. I wanted to keep it compact to simplify "copy + paste"
# usage.
#
# * This script is made to operate in two CMake contexts: 1. Configure time
#   context (when build files are created). 2. Build time context (called via
#   CMake -P) If you see something odd (e.g. the NOT DEFINED clauses), consider
#   that it can run in one of two contexts.
...
# the state of the git repo changes (e.g. a commit is made).
...
# executed. _state       (out) list; a collection of variables representing the
# state of the repository (e.g. commit SHA).
function(GetGitState _working_dir _state)
  ...
  execute_process(
    COMMAND "${GIT_EXECUTABLE}" status --branch --short --porcelain
    WORKING_DIRECTORY "${_working_dir}"
    RESULT_VARIABLE res
  ...
  execute_process(
    COMMAND "${GIT_EXECUTABLE}" --no-pager log -n 1
    WORKING_DIRECTORY "${_working_dir}"
    RESULT_VARIABLE res
...
# the state of the repo has changed. _state         (out)    list; the
# repository state as a list (e.g. commit SHA).
function(CheckGit _working_dir _state_changed _state)

  # Get the current state of the repo.
  getgitstate("${_working_dir}" state)

  # Set the output _state variable. (Passing by reference in CMake is
  # awkward...)
  set(${_state}
      ${state}
      PARENT_SCOPE)
