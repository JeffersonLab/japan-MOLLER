...
# ----------------------------------------------------------------------------
# Get list of definitions for given target as list of -DXXX (needed for
...
macro(set_compiler_flags _suggested_flags)
  ...
  check_cxx_compiler_flag(-fomit-frame-pointer
                          cxx-compiler-supports-fomit-frame-pointer)
  ...
  if(cxx-compiler-supports-fsigned-char)
    set(${PROJECT_NAME_UC}_CXX_FLAGS
        "${${PROJECT_NAME_UC}_CXX_FLAGS} -fsigned-char")
  endif()
  if(cxx-compiler-supports-mtune-generic)
    set(${PROJECT_NAME_UC}_CXX_FLAGS
        "${${PROJECT_NAME_UC}_CXX_FLAGS} -mtune=generic")
  endif()
  if(cxx-compiler-supports-Qunused-arguments)
    set(${PROJECT_NAME_UC}_CXX_FLAGS
        "${${PROJECT_NAME_UC}_CXX_FLAGS} -Qunused-arguments")
  endif()
  set(${PROJECT_NAME_UC}_CXX_FLAGS
      "${${PROJECT_NAME_UC}_CXX_FLAGS} ${_suggested_flags}")
  if(${PROJECT_NAME_UC}_CXX_FLAGS)
    remove_duplicates(${${PROJECT_NAME_UC}_CXX_FLAGS}
                      ${PROJECT_NAME_UC}_CXX_FLAGS)
    set_cxx_std(${${PROJECT_NAME_UC}_CXX_FLAGS})
  endif()
  string(STRIP "${${PROJECT_NAME_UC}_CXX_FLAGS}"
...
macro(set_diagnostic_flags)
  ...
  if(SDF_WERROR)
    set(${PROJECT_NAME_UC}_DIAG_FLAGS
        "${${PROJECT_NAME_UC}_DIAG_FLAGS} -Werror")
  endif()
  if(${ARGC} GREATER 0)
    remove_duplicates(${${PROJECT_NAME_UC}_DIAG_FLAGS}
                      ${PROJECT_NAME_UC}_DIAG_FLAGS)
  endif()
  string(STRIP "${${PROJECT_NAME_UC}_DIAG_FLAGS}"
...
function(report_build_info)
  ...
  message(
    STATUS
      "Compiling for ${CMAKE_SYSTEM_NAME} with ${CMAKE_CXX_COMPILER} version ${CMAKE_CXX_COMPILER_VERSION}"
  )
  message(STATUS "Build type ${CMAKE_BUILD_TYPE}")
  message(
    STATUS
      "${PROJECT_NAME_UC}_CXX_FLAGS = ${${PROJECT_NAME_UC}_CXX_FLAGS} ${${PROJECT_NAME_UC}_DIAG_FLAGS}"
  )
  message(
    STATUS
      "CMAKE_CXX_FLAGS_${BUILD_TYPE_UC} = ${CMAKE_CXX_FLAGS_${BUILD_TYPE_UC}}")
...
function(config_add_dependency item)
  ...
  if(${found} LESS 0)
    set_property(GLOBAL APPEND PROPERTY ${PROJECT_NAME_UC}_DEPENDENCY_LIST
                                        ${item})
    get_property(commands GLOBAL
                 PROPERTY ${PROJECT_NAME_UC}_FIND_DEPENDENCY_COMMANDS)
