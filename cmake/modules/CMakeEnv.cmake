...
macro(set_compiler_flags _suggested_flags)
  ...
  check_cxx_compiler_flag(-march=generic cxx-compiler-supports-march-generic)
  check_cxx_compiler_flag(-march=native cxx-compiler-supports-march-native)
  check_cxx_compiler_flag(-mtune=generic cxx-compiler-supports-mtune-generic)
  check_cxx_compiler_flag(-mtune=native cxx-compiler-supports-mtune-native)
  if(APPLE)
  ...
  # Combine all existing flag sources to check for user-provided -march/-mtune.
  # If the user already passed explicit -march= or -mtune= via CMAKE_CXX_FLAGS,
  # CXXFLAGS, or the suggested flags, do not override them with native/generic.
  set(_all_existing_flags "${_suggested_flags} ${CMAKE_CXX_FLAGS} $ENV{CXXFLAGS}")
  if(CMAKE_BUILD_TYPE STREQUAL "Release" OR CMAKE_BUILD_TYPE STREQUAL "RelWithDebInfo")
    if(NOT "${_all_existing_flags}" MATCHES "-march=")
      if(cxx-compiler-supports-march-native)
        set(${PROJECT_NAME_UC}_CXX_FLAGS "${${PROJECT_NAME_UC}_CXX_FLAGS} -march=native")
      endif()
    endif()
    if(NOT "${_all_existing_flags}" MATCHES "-mtune=")
      if(cxx-compiler-supports-mtune-native)
        set(${PROJECT_NAME_UC}_CXX_FLAGS "${${PROJECT_NAME_UC}_CXX_FLAGS} -mtune=native")
      endif()
    endif()
  else()
    if(NOT "${_all_existing_flags}" MATCHES "-march=")
      if(cxx-compiler-supports-march-generic)
        set(${PROJECT_NAME_UC}_CXX_FLAGS "${${PROJECT_NAME_UC}_CXX_FLAGS} -march=generic")
      endif()
    endif()
    if(NOT "${_all_existing_flags}" MATCHES "-mtune=")
  ...
  unset(cxx-compiler-supports-mtune-native)
