...
#
# (a) ROOT 5 does not provide a CMake configuration module, and we still want to
# support ROOT 5 for the moment; and (b) ROOT 6's ROOTConfig.cmake is a bit too
# heavy for our purposes.
#
# Among other things, ROOT 6's ROOTConfig-targets.cmake, which is invoked by
# ROOTConfig.cmake, requires all components to be present, which clashes with
# partial RPM installs from EPEL on RHEL. Using ROOT's CMake configuration
# always requires users to install all components built by the packager, even
# those that are never needed. We don't want that.

if(NOT TARGET ROOT::Libraries)

  if(NOT ROOT_CONFIG_EXEC)
    # Only execute this if not already done (spawning a bunch of root-config
    # processes is quite slow)

    # Find root-config either in $CMAKE_PREFIX_PATH/bin, $ROOTSYS/bin, if
    # defined, or in $PATH.
    if(DEFINED ENV{ROOTSYS})
      set(ROOTBIN $ENV{ROOTSYS}/bin)
    endif()
    find_program(
      ROOT_CONFIG_EXEC root-config
      HINTS "${ROOTBIN}"
    ...
    if(ROOT_CONFIG_EXEC)
      ...
      set(ROOTSYS
          ${_rootsys}
          CACHE PATH "ROOT installation prefix" FORCE)
    ...
    execute_process(
      COMMAND ${ROOT_CONFIG_EXEC} --version
      OUTPUT_VARIABLE _config_version
      OUTPUT_STRIP_TRAILING_WHITESPACE)
    string(REGEX REPLACE "^([0-9]+)\\.([0-9]+)/([0-9]+).*" "\\1.\\2.\\3"
                         _version "${_config_version}")
    set(ROOT_CONFIG_VERSION
        ${_config_version}
        CACHE STRING "ROOT version from root-config" FORCE)
    set(ROOT_VERSION
        ${_version}
        CACHE STRING "ROOT version found" FORCE)
    ...
    set(ROOT_BINARY_DIR
        "${_bindir}"
        CACHE PATH "ROOT binary directory" FORCE)
    unset(_bindir)
    ...
    set(ROOT_LIBRARY_DIR
        "${_libdir}"
        CACHE PATH "ROOT library directory" FORCE)
    unset(_libdir)
    ...
    set(ROOT_INCLUDE_DIR
        "${_incdir}"
        CACHE PATH "ROOT include directory" FORCE)
    unset(_incdir)
    ...
    execute_process(
      ...
      OUTPUT_STRIP_TRAILING_WHITESPACE)
    # Remove include directories from compiler flags; they are handled
    # separately
    string(REGEX REPLACE "-I[^ ]*" "" _cxx_flags "${_cxx_flags}")
    set(ROOT_CXX_FLAGS
        "${_cxx_flags}"
        CACHE STRING "ROOT C++ compiler flags" FORCE)
    ...
    set(ROOT_LD_FLAGS
        "${_ldflags}"
        CACHE STRING "ROOT linker flags" FORCE)
    ...
    set(ROOT_LIB_FLAGS
        "${_libs}"
        CACHE STRING "Linker flags for ROOT libraries" FORCE)
  ...
  string(REGEX MATCH "(^| +)-std=([^ ]*)\\+\\+(..)" _cxx_std
               "${ROOT_CXX_FLAGS}")
  set(uselibnew TRUE)
  ...
  set_target_properties(ROOT::Libraries PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                                   "${ROOT_INCLUDE_DIRS}")
  set_target_properties(ROOT::Libraries PROPERTIES INTERFACE_LINK_LIBRARIES
                                                   "${targetlist}")
  unset(targetlist)

  # If any requested components are remaining, they weren't found above, so
...
if(NOT MK_ROOTDICT)
  message(
    FATAL_ERROR "FindROOT: Cannot find mk_rootdict.sh. Check your installation."
  )
endif()

# BUILD_ROOT_DICTIONARY(dictionary LINKDEF <theLinkDef.h> [ TARGETS <target> [
# <target> ... ] [ INCLUDEDIRS <dir> [ <dir ... ] [ OPTIONS [ <options> ] [
# PCMNAME <pcmname> ] <header> [ <header> ... ] )
#
# Build ROOT dictionary for ROOT 5 and 6
#
# Arguments: dictionary:    dictionary base name (required). Output file will be
# ${dictionary}Dict.cxx LINKDEF:       name of LinkDef.h file (required)
# TARGETS:       CMake targets from which to get -I arguments and -D definitions
# INCLUDEDIRS:   paths to use for -I arguments OPTIONS:       additional options
# for rootcling/rootcint PCMNAME:       base name of PCM file (ROOT 6 only).
# Defaults to ${dictionary}. Output will be lib${pcmname}_rdict.pcm.
#
# Creates a custom target ${dictionary}_ROOTDICT. With ROOT 6, also installs the
# PCM file in ${CMAKE_INSTALL_LIBDIR}
#
function(build_root_dictionary dictionary)
  ...
  cmake_parse_arguments(RGD "" "PCMNAME" "INCLUDEDIRS;LINKDEF;OPTIONS;TARGETS"
                        ${ARGN})

  if(NOT RGD_LINKDEF)
    message(
      FATAL_ERROR "root_generate_dictionary: Required argument LINKDEF missing")
  endif()
  ...
  if(ROOTCLING)
    ...
    add_custom_command(
      ...
      COMMAND
        ${MK_ROOTDICT} ARGS ${ROOTCLING} -f ${dictionary}Dict.cxx -s
        lib${pcmname} ${RGD_OPTIONS} INCDIRS "${incdirs}" DEFINES "${defines}"
        ${RGD_UNPARSED_ARGUMENTS} ${RGD_LINKDEF}
      VERBATIM
    ...
    add_custom_command(
      ...
      COMMAND
        ${MK_ROOTDICT} ARGS ${ROOTCINT} -f ${dictionary}Dict.cxx -c
        ${RGD_OPTIONS} INCDIRS "${incdirs}" DEFINES "${defines}"
        ${RGD_UNPARSED_ARGUMENTS} ${RGD_LINKDEF}
      VERBATIM
      DEPENDS ${RGD_UNPARSED_ARGUMENTS} ${RGD_LINKDEF})
  endif()
  add_custom_target(${dictionary}_ROOTDICT DEPENDS ${dictionary}Dict.cxx)
endfunction()
