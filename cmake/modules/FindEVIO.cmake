...
if(DEFINED ENV{EVIO})
  set(evio_arch_lib $ENV{EVIO}/${evio_arch}/lib
                    $ENV{EVIO}/${CMAKE_SYSTEM_NAME}/lib)
  set(evio_arch_inc $ENV{EVIO}/${evio_arch}/include $ENV{EVIO}/common/include)
endif()
if(DEFINED ENV{CODA})
  set(coda_arch_lib $ENV{CODA}/${evio_arch}/lib
...
find_library(
  EVIO_LIBRARY ${evio_dbg} evio
  PATHS ENV EVIO_LIBDIR ${evio_arch_lib} ${coda_arch_lib} NO_DEFAULT_PATHS
  DOC "EVIO C-API library")
find_library(EVIO_LIBRARY ${evio_dbg} evio DOC "EVIO C-API library")
find_path(
  EVIO_INCLUDE_DIR
  NAMES evio.h
...
if(EVIO_INCLUDE_DIR)
  ...
  file(STRINGS "${EVIO_INCLUDE_DIR}/evio.h" VERSION_STRING
       REGEX "${VERSION_REGEX}")
  string(REGEX REPLACE "${VERSION_REGEX}" "\\1" EVIO_VERSION
                       "${VERSION_STRING}")
endif()
if(NOT TARGET EVIO::EVIO
   AND EVIO_LIBRARY
   AND EVIO_INCLUDE_DIR)
  add_library(EVIO::EVIO SHARED IMPORTED)
  set_target_properties(
    EVIO::EVIO PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${EVIO_INCLUDE_DIR}"
