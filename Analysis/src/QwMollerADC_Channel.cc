...
void QwMollerADC_Channel::SetEventData(const Double_t* block, UInt_t sequencenumber)
{
  fHardwareBlockSum = 0.0;
  fHardwareBlockSumM2 = 0.0; // second moment is zero for single events
  for (Int_t i = 0; i < fBlocksPerEvent; i++) {
...
void QwMollerADC_Channel::EncodeEventData(std::vector<UInt_t> &buffer)
{
  ...
  if (IsNameEmpty()) {
    ...
    //    localbuf[4] = 0;
    for (Int_t i = 0; i < 4; i++) {
      localbuf[static_cast<ptrdiff_t>(i*5)] = fBlock_raw[i];
...
Int_t QwMollerADC_Channel::ProcessEvBuffer(UInt_t* buffer, UInt_t num_words_left, UInt_t index)
{
  ...
  if (IsNameEmpty()){
  ...
  } else if (num_words_left >= fNumberOfDataWords)
    {
      ...
      fSoftwareBlockSum_raw = 0;
      for (Int_t i=0; i<fBlocksPerEvent; i++){
        fBlock_raw[i] = localbuf_signed[static_cast<ptrdiff_t>(i*5)];
...
void  QwMollerADC_Channel::ConstructHistograms(TDirectory *folder, TString &prefix)
{
  ...
  if (IsNameEmpty()){
    ...
    if(fDataToSave==kRaw)
      {
        fHistograms.resize(8+2+1, nullptr);
        size_t index=0;
        for (Int_t i=0; i<fBlocksPerEvent; i++){
      ...
      {
        fHistograms.resize(4+1+1, nullptr);
        Int_t index=0;
        for (Int_t i=0; i<fBlocksPerEvent; i++){
            ...
            for (Int_t i=0; i<fBlocksPerEvent; i++)
              {
                ...
                if (fHistograms[index+1] != nullptr && (fErrorFlag)==0) {
...
{
  const QwMollerADC_Channel* tmpptr = nullptr;
...
QwMollerADC_Channel QwMollerADC_Channel::operator+ (const QwMollerADC_Channel &value) const
{
...
{
  const auto* input = dynamic_cast<const QwMollerADC_Channel*>(value);
