...
void  QwVQWK_Channel::ConstructNTupleAndVector(std::unique_ptr<ROOT::RNTupleModel>& model, TString &prefix, std::vector<Double_t> &values, std::vector<std::shared_ptr<Double_t>> &fieldPtrs)
{
  ...
  if (bHw_sum) {
    ...
    if (fDataToSave == kMoments) {
      values.push_back(0.0);
      auto field_m2 = model->MakeField<Double_t>(TString::Format("%s_hw_sum_m2", basename.Data()).Data());
      fieldPtrs.push_back(field_m2);
      
      values.push_back(0.0);
      auto field_err = model->MakeField<Double_t>(TString::Format("%s_hw_sum_err", basename.Data()).Data());
      fieldPtrs.push_back(field_err);
    }
  }
