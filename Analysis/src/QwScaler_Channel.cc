...
template<unsigned int data_mask, unsigned int data_shift>
void QwScaler_Channel<data_mask,data_shift>::ConstructNTupleAndVector(std::unique_ptr<ROOT::RNTupleModel>& model, TString& prefix, std::vector<Double_t>& values, std::vector<Double_t*>& fieldPtrs)
{
  if (IsNameEmpty()){
    ...
    if(fDataToSave==kRaw){
      ...
      if ((~data_mask) != 0){
        fieldPtrs.push_back(&values[index++]);
        model->MakeField<Double_t>((basename + "_header").Data());
      }
    }
